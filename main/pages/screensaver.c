...
static void start_cycle(void) {
  int32_t x = esp_random() % (SCREEN_SIZE - LOGO_SIZE);
  int32_t y = esp_random() % (SCREEN_SIZE - LOGO_SIZE);
...
void screensaver_create(lv_obj_t *parent, screensaver_dismiss_cb_t cb) {
  ...
  lv_obj_set_size(scr_container, SCREEN_SIZE, SCREEN_SIZE);
  theme_apply_screen(scr_container);

  // kern_logo_create children: [0]=outer ring, [1]=inner ring, [2]=core
  logo = kern_logo_create(scr_container, 0, 0, LOGO_SIZE);
